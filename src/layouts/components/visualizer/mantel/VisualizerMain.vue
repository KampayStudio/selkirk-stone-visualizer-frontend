...
const props = defineProps(['image', 'mantel'])
...
const drawShapes = () => {
  ...
  props.mantel.wall_shape.shapes.forEach((shape, index) => {
...
const wallClicked = async (shape, index) => {
  ...
  isLoadingOpen.value = !isLoadingOpen.value
  router.replace(route.query.to ? String(route.query.to) : '/visualizer/mantel/visualize-mantel')
}
</script>
...
<template>
  <div class="d-flex justify-center">
    <div class="image-container">
      ...
      >
      <canvas
        v-for="(shape, index) in mantel.wall_shape.shapes"
        :key="index"
        ref="canvasRefs"
        class="shape-canvas"
